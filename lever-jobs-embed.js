    ...
    for(var i = 0; i < groupedPostings.length; i++) {
      ...
      for (j = 0; j < groupedPostings[i].teams.length; j ++) {
        ...
        for (var k = 0; k < groupedPostings[i].teams[j].postings.length; k ++) {
          ...
          '<a class="lever-job-title" href="' + groupedPostings[i].teams[j].postings[k].hostedUrl + '"><strong>' + sanitizeForHTML(groupedPostings[i].teams[j].postings[k].text), '' + '</strong><span class="lever-job-tag">' + (sanitizeForHTML(groupedPostings[i].teams[j].postings[k].categories.location) || '') + '</span></a></li>';
        }
